...
if [ -n "$ARCHIVE_PATH" ]; then
  ...
  if ! command -v curl >/dev/null 2>&1; then
    echo "curl was not found. Install curl before running this uninstaller." >&2
    exit 1
  fi
...
if [ -f "$EXTRACT_DIR/uninstall-doublecheck.sh" ]; then
  REPO_DIR="$EXTRACT_DIR"
else
  REPO_DIR=$(find "$EXTRACT_DIR" -mindepth 1 -maxdepth 1 -type d | head -n 1)
fi

if [ -z "${REPO_DIR:-}" ] || [ ! -f "$REPO_DIR/uninstall-doublecheck.sh" ]; then
  echo "Could not find uninstall-doublecheck.sh in downloaded archive." >&2
  exit 1
fi

sh "$REPO_DIR/uninstall-doublecheck.sh"
